...
BIRMINGHAM_COORDINATES = {"latitude": 52.4862, 
                          "longitude": -1.8904}
...
def generate_weather_data(device_id, timestamp, location):
    return {
        ...
        'airQualityIndex': random.uniform(0, 100) # AQL value goes here
    }
...
def delivery_report(err, msg):
    if err is not None:
        print(f"Delivery of message failed: {err}")
    else:
...
def simulate_journey(producer, device_id):
    while True:
        ...
        traffic_camera_data = generate_traffic_camera_data(device_id, vehicle_data['timestamp'], vehicle_data['location'], 'Sony-Camera')
...
if __name__ == "__main__":
    ...
    try:
        simulate_journey(producer, 'Vehicle-OmkarYeole')
